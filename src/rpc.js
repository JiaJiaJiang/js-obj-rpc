...
class Packer{		//data packer
	static pack(packType,id,data){
		...
		switch(data){
			...
			default:{//data which has data body
...
class Pack{		//data pack
	constructor(buffer){
		...
		this._dataType=(this.head>>3&0b111);
		this.idByte=this.head&0b111;idBytes
		if(this.requireResponse&&this.idByte){
			let idView=new DataView(this.buffer,this.idByte===3?0:1,this.idByte===3?4:this.idByte);
			switch(this.idByte){
				case 1:{this.id=idView.getUint8();break;}
				case 2:{this.id=idView.getUint16();break;}
				case 3:{this.id=(idView.getUint32()&0xFF000000);break;}
	...
	getData(){
		switch(this._dataType){
			...
			case 5:return null;
			case 6:{
				return JSON.parse(this._readString());
			}
			case 7:{
				return (new DataView(this._dataBuffer(),0,4)).getInt32();
			}
		}
	}
	_dataBuffer(){
...
class RPC{		//RPC handle
	...
	/*------------------------*/
	data(datapack){}//rewrite this method to resolve data
}
